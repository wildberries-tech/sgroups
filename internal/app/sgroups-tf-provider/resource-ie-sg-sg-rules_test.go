...
import (
	...
	"github.com/wildberries-tech/sgroups/v2/internal/app/sgroups-tf-provider/fixtures"
	domain "github.com/wildberries-tech/sgroups/v2/internal/domains/sgroups"
		...
		resourceTestCase.Steps = append(resourceTestCase.Steps, resource.TestStep{
			...
			Check: func(_ *terraform.State) error {
				if len(expectedBackend)+len(nonExpectedBackend) > 0 {
					...
					var checker fixtures.ExpectationsChecker[protos.IESgSgRule, domain.IESgSgRule]
...
func (sui *ieSgSgRulesTests) listAllRules() []*protos.IESgSgRule {
	resp, err := sui.sgClient.FindIESgSgRules(sui.ctx, &protos.FindIESgSgRulesReq{
		SgLocal: []string{},
		SG:      []string{},
	})
