...
import (
	...
	"github.com/wildberries-tech/sgroups/v2/internal/app/agent"
	"github.com/wildberries-tech/sgroups/v2/internal/app/agent/nft/resources"
	model "github.com/wildberries-tech/sgroups/v2/internal/domains/sgroups"
...
type (
	...
	batch struct {
		...
		dnsResolver  agent.DomainAddressQuerier
		fqdnStrategy agent.FqdnRulesStrategy
		table        *nftlib.Table
...
func (bt *batch) initBaseRules(dir direction) {
	...
	for i, nw := range sli(resources.SeparateNetworks(nws)) {
		isIP4 := i == 0
		nw := nw
...
func (bt *batch) addSGNetSets() {
	bt.data.Networks.IterateNetworks(func(sgName string, nws []model.Network) bool {
		nwsV4, nwsV6 := resources.SeparateNetworks(nws)
...
func (bt *batch) addFQDNNetSets() {
	if !bt.fqdnStrategy.Eq(agent.FqdnRulesStartegyDNS) {
		return
	}
	f := func(IPv int, domain model.FQDN, a agent.DomainAddresses) {
	...
	bt.data.ResolvedFQDN.A.Iterate(func(domain model.FQDN, a agent.DomainAddresses) bool {
		f(iplib.IP4Version, domain, a)
		return true
	})
	bt.data.ResolvedFQDN.AAAA.Iterate(func(domain model.FQDN, a agent.DomainAddresses) bool {
...
func (gp *jobGroup) populateOutSgFqdnRules(sg *resources.SG) {
	const (
		useDNS = 1 << iota
		//useNDPI
	)
	...
	for _, ipV := range sli(model.IPv4, model.IPv6) {
		...
		for _, rule := range rules {
			...
			pri := rule.Priority.SomeOr(
				resources.RuleBasePriority(rule),
			)
...
func (bt *batch) populateDefaultIcmpRules(dir direction, sg *resources.SG) {
...
func (gp *jobGroup) populateInOutSgIcmpRules(dir direction, sg *resources.SG) {
	targetChName := nameUtils{}.nameOfInOutChain(dir, sg.Name)
	isIN := dir == dirIN
	bt := gp.bt
	rules := tern(isIN,
...
func (gp *jobGroup) populateSgIeSgIcmpRules(dir direction, sg *resources.SG) {
	isIN := dir == dirIN
	bt := gp.bt
...
func (gp *jobGroup) populateIeCidrSgIcmpRules(dir direction, sg *resources.SG) {
	isIN := dir == dirIN
	bt := gp.bt
...
func (gp *jobGroup) populateInOutSgRules(dir direction, sg *resources.SG) {
	targetSGchName := nameUtils{}.nameOfInOutChain(dir, sg.Name)
	isIN := dir == dirIN
	bt := gp.bt
...
func (gp *jobGroup) populateInOutSgIeSgRules(dir direction, sg *resources.SG) {
	isIN := dir == dirIN
	bt := gp.bt
...
func (gp *jobGroup) populateInOutCidrSgRules(dir direction, sg *resources.SG) {
	isIN := dir == dirIN
	bt := gp.bt
...
func (bt *batch) makeInOutChains(dir direction) {
	bt.data.LocalSGs.Iterate(func(_ string, sg *resources.SG) bool {
...
func (bt *batch) chainInOutProlog(dir direction, sg *resources.SG) {
...
func (bt *batch) chainInOutEpilog(dir direction, sg *resources.SG) {
