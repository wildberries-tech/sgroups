...
import (
	...
	conv "github.com/wildberries-tech/sgroups/v2/internal/api/sgroups"
	...
	sgAPI "github.com/wildberries-tech/sgroups/v2/pkg/api/sgroups"
...
type (
	...
	ResolvedFQDN struct {
		sync.RWMutex
		A    dict.RBDict[model.FQDN, agent.DomainAddresses]
...
// UpdA -
func (r *ResolvedFQDN) UpdA(domain model.FQDN, addr agent.DomainAddresses) {
	r.Lock()
	defer r.Unlock()
	r.A.Put(domain, addr)
}
...
// UpdAAAA -
func (r *ResolvedFQDN) UpdAAAA(domain model.FQDN, addr agent.DomainAddresses) {
	r.Lock()
	defer r.Unlock()
...
// Resolve -
func (r *ResolvedFQDN) Resolve(ctx context.Context, rules SG2FQDNRules, dnsRes agent.DomainAddressQuerier) {
	const parallelism = 7

	type item = struct {
		domain model.FQDN
		up     func(model.FQDN, agent.DomainAddresses)
		re     func(context.Context, string) agent.DomainAddresses
	}
	var items []item
