...
<table>
    ...
    <tbody>
        ...
        <tr>
            <td>\{RuleType\}</td>
            <td>ip</td>
            <td>Значение для исходящего трафика в цепочке правил.</td>
            <td>Описывает, что отправляет трафик типа ip</td>
        </tr>
        <tr>
            <td>\{SrcPorts\}</td>
            <td>sport {}</td>
...
chain FW-OUT-sgname_example {
  # **********
  nftrace set 1 ip daddr { 1.1.1.1/8 } tcp dport { 80, 443 } counter packets 0 bytes 0 log level debug flags ip options accept
  # **********
}
```