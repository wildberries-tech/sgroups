...
<Tabs
    ...
    values= {[
        ...
        { label: "FW-OUT-POSTROUTING", value: "fw-out-postrouting" },
...
>
    ...
    <TabItem value="fw-out-postrouting">
        #### Описание

        <div  className="text-justify">
          Правило перехода в цепочку `FW-OUT-POSTROUTING-$sgName` с проверкой что трафик является исходящим и предназначен для 
        ...
        ```hcl
          ...
          chain FW-OUT-POSTROUTING-no-routed {
              ct state established,related counter packets 0 bytes 0 accept
              # ******
              counter packets 0 bytes 0 accept
          }
