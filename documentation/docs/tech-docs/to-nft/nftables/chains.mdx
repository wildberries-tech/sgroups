...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>
        <nobr>FW-EGRESS-POSTROUTING-$sgName</nobr>
      </td>
      <td>Egress</td>
      <td className="text-justify">
        <i>Вторая</i> цепочка содержит наборы правил, относящиеся только к конкретной Security Group.
...
<Tabs
    defaultValue="fw-ingress-input"
    values= {[
        { label: "FW-INGRESS-INPUT", value: "fw-ingress-input" },
        { label: "FW-EGRESS-POSTROUTING", value: "fw-egress-postrouting" },
        {/* { label: "FW-OUT-POSTROUTING (DPI)", value: "fw-out-postrouting-dpi" } */}
    ]}
>

    <TabItem value="fw-ingress-input">
        #### Описание
        <div  className="text-justify">
          Правило перехода в цепочку `FW-INGRESS-INPUT-sgName` с проверкой что трафик является входящим и предназначен для указанной Security Group.
        </div>
        ...
        ```hcl
        chain FW-INGRESS-INPUT{
            type filter $Hook $HookPriority; policy accept;
            # **********
            $Trace $RuleType $DstSgroup $Counter $Verdict FW-IN-$sgName
            # **********
            $Counter drop
        }
        ```
        ...
        chain FW-INGRESS-INPUT{
            ...
            # **********
            nftrace set 1 ip daddr @NetIPv4-exampleSG counter packets   0 bytes 0 goto FW-IN-INPUT-exampleSG
            # **********
        ...
        table inet main-1705582480 {
            ...
            chain FW-INGRESS-INPUT {
                ...
                # ******
                nftrace set 1 ip daddr @NetIPv4-no-routed counter packets   0 bytes 0 goto FW-IN-INPUT-no-routed
                nftrace set 1 ip daddr @NetIPv4-exampleSG counter packets   0 bytes 0 goto FW-IN-INPUT-exampleSG
                counter packets 0 bytes 0 drop
            }

            chain FW-INGRESS-INPUT-no-routed {
                ct state established,related counter packets 0 bytes 0 accept
                # ******
                counter packets 0 bytes 0 accept
            }

            chain FW-INGRESS-INPUT-exampleSG {
                ct state established,related counter packets 0 bytes 0 accept
                # ******
                counter packets 0 bytes 0 accept
            }
    ...
    <TabItem value="fw-egress-postrouting">
        #### Описание

        <div  className="text-justify">
          Правило перехода в цепочку `FW-EGRESS-POSTROUTING-$sgName` с проверкой что трафик является исходящим и предназначен для
        ...
        ```hcl
        chain FW-EGRESS-POSTROUTING {
            type filter $Hook $HookPriority; policy accept;
            # **********
            $Trace $RuleType $SrcSgroup $Counter $Verdict FW-OUT-POSTROUTING-$sgName
            # **********
            $Counter drop
        }
        ```
        ...
        chain FW-EGRESS-POSTROUTING {
            ...
            # **********
            nftrace set 1 ip saddr @NetIPv4-exampleSG counter packets 0 bytes 0 goto FW-OUT-POSTROUTING-exampleSG
            # **********
            counter packets 0 bytes 0 drop
        }
        ```
        ...
        ```hcl
          ...
          chain FW-EGRESS-POSTROUTING {
              ...
              # **********
              nftrace set 1 ip saddr @NetIPv4-exampleSG counter packets 0 bytes 0 goto FW-OUT-POSTROUTING-exampleSG
              nftrace set 1 ip saddr @NetIPv4-no-routed counter packets 0 bytes 0 goto FW-OUT-POSTROUTING-no-routed
              counter packets 0 bytes 0 drop
          }

          chain FW-EGRESS-POSTROUTING-no-routed {
              ct state established,related counter packets 0 bytes 0 accept
              # ******
              counter packets 0 bytes 0 accept
          }

          chain FW-EGRESS-POSTROUTING-exampleSG {
              ct state established,related counter packets 0 bytes 0 accept
              # ******
              counter packets 0 bytes 0 accept
          }
