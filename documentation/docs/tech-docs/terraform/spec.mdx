...
<dev>
    После установки провайдера, пользователь может приступать к описанию собственных sgroups.
    В качестве отправной точки, воспользуемся готовым шаблоном.
</dev>
...
<dev>
    ...
    <ul>
        <li>
            Убедитесь, что вы корректно настроили параметр <a href='https://github.com/H-BF/swarm-spec-template/blob/master/main.tf#L2'>source</a>. Важной часть параметра, является 
    ...
    Воспользуйтесь предоставленными в репозитории sgroups для проверки работоспособности провайдера. Для этого выполните команду:
    ...
    Для того, чтобы применить описанные правила, выполните команду:
