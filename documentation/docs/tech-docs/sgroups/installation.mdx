...
<TabItem value='docker'>
  ...
  ```
    ...
    <dev>
        В случае запуска в режиме "Postgres" корректно установите переменную окружения `SG_STORAGE_POSTGRES_URL` командой:
    </dev>

    <dev>
        Далее запустите запустите hbf-server командой:
    </dev>
    ```bash
    sh setup.sh
    docker run \
