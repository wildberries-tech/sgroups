  ...
  <table>
    ...
    <tbody>
      ...
      <tr>
        ...
        <td>Включить/Отключить возможность получения информации о статусе hbf-сервера по `/healthcheck`. Значение по умолчанию: `true`.</td>
        <td>Boolean</td>
        <td>Нет</td>
      ...
      <tr>
        ...
        <td>String</td>
        <td>При использовании `storage.type:postgres`, данный параметр должно иметь корректный URL подключения. В ином случае, параметр необязательный для заполнения.</td>
