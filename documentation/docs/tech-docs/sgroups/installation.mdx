...
<dev>
  ...
  <ul>
      ...
      <li>
        Postgres: В этом случае пользователю необходимо перед запуском hbf-server выполнить [миграцию базы данных](./goose.mdx).
      </li>
  </ul>
</dev>
...
{' '}
  ...
  ```
  <dev>
    В случае запуска в режиме "In Memory" установите переменную `SG_STORAGE_TYPE` командой:
  </dev>
