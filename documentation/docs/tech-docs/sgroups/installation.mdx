...
<ul>
    <li>
      In Memory: пользователю не надо дополнительно ничего настраивать.
    </li>
    <li>
      Postgres: пользователю необходимо перед запуском сервера выполнить [миграцию базы данных](./goose.mdx). Также необходимо убедиться,
      что переменная окружения `SG_STORAGE_POSTGRES_URL` установлена.
    </li>
</ul> 
...
<TabItem value='docker'>
    ...
    ```bash
    docker build -f Dockerfile.server --tag sgroups:v$(cat VERSION) .
    ```
    ...
    <dev>
        В случае запуска в режиме "Postgres" корректно установите переменную окружения `SG_STORAGE_POSTGRES_URL` и только после этого запустите 
        hbf-server командой:
    </dev>
...
<TabItem value='source'>
  ...
  <dev>
    В случае запуска в режиме "Postgres" корректно установите переменную `SG_STORAGE_POSTGRES_URL`.
  </dev>
