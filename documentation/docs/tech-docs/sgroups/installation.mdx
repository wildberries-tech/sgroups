...
<ul>
    ...
    <li>
      Postgres: пользователю необходимо перед запуском сервера выполнить миграцию базы данных. Также необходимо убедиться,
...
{' '}
  ...
  <dev>
    В случае запуска в режиме "Postgres" корректно установите переменную `SG_STORAGE_POSTGRES_URL` командой:
  </dev>

  ```bash
  export SG_STORAGE_POSTGRES_URL=""
  ```
