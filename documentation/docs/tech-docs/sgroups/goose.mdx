...
<div className="paragraph">
  <i>Goose</i> представляет собой инструмент для миграции баз данных. Обычно он используется перед запуском серверной
  части приложения. Более подробную информацию о <i>goose</i> можно найти [здесь](https://github.com/pressly/goose).
</div>
...
>
  <TabItem value="bin">
        ...
        <div className="paragraph">
        Для сборки <i>goose</i> из корневой директории репозитория выполните команду: `make .install-goose`. После сборки,
        скомпилированный бинарный файл будет доступен в папке `./bin/goose` в корне репозитория (не путать с `/bin`,
