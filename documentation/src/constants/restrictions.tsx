...
const template_s2s = `"$\{transport\}:sg-local($\{sg_local\})sg($\{sg\})"`;
const template_s2s_ie = `"$\{transport\}:sg-local($\{sg_local\})sg($\{sg\})$\{traffic\}"`
const template_s2c_ie = `"$\{transport\}:cidr($\{cidr\})sg($\{sg_name\})$\{traffic\}"`
const template_s2f_e = `"$\{transport\}:sg($\{sg_from\})fqdn($\{fqdn\})"`
...
export const RESTRICTIONS: TRestrictions = {
    ...
    cidr: [
        ...
        <>
            Подсеть должна соответствовать формату записи, определенному в RFC <a href="https://datatracker.ietf.org/doc/html/rfc4632">4632</a>.
        </>
    ],
    ...
    itemsRuleName_s2s: [
        <>
            Форма ruleName должна быть организована в соответствии с определенной последовательностью, которую нужно соблюдать <code>{template_s2s}</code>.
        </>
    ],

    itemsRuleName_s2s_ie: [
    <>
        Форма ruleName должна быть организована в соответствии с определенной последовательностью, которую нужно соблюдать <code>{template_s2s_ie}</code>.
    </>
    ],

    itemsRuleName_s2c_ie: [
        <>
            Форма ruleName должна быть организована в соответствии с определенной последовательностью, которую нужно соблюдать <code>{template_s2c_ie}</code>.
        </>        
    ],

    itemsRuleName_s2f_e: [
        <>
            Форма ruleName должна быть организована в соответствии с определенной последовательностью, которую нужно соблюдать <code>{template_s2f_e}</code>.
        </>        
    ],
}